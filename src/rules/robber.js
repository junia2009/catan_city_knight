...
// targetPlayer の手札からランダムに1枚を pid へ移す。商品も対象(cak)。
// 相手が手札0枚なら何もしない。
export function stealRandomCard(state, pid, targetPlayer) {
...
export function applyRobberMove(state, pid, hexId, targetPlayer) {
  ...
  if (targetPlayer != null) {
    stealRandomCard(state, pid, targetPlayer);
    addLog(state, `${p.name}が盗賊を移動し、${state.players[targetPlayer].name}から1枚奪いました`);
  } else {
